package com.wu.wuaicode.langgraph4j.tools;
...
import com.wu.wuaicode.langgraph4j.model.ImageCategoryEnum;
...
public class UndrawIllustrationTool {
    ...
    public List<ImageResource> searchIllustrations(@P("搜索关键词") String query) {
        ...
        try (HttpResponse response = HttpRequest.get(apiUrl).setHttpProxy("127.0.0.1", 7890).timeout(10000).execute()) {
            if (!response.isOk()) {
                return imageList;
            }
